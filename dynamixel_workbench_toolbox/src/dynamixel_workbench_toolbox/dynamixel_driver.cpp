...
void DynamixelDriver::setTools(uint16_t model_number, uint8_t id) {
    if (tools_cnt_ == 0) {
        ...
        if (!strncmp(tools_[tools_cnt_ - 1].dxl_info_[0].model_name, findModelName(model_number),
...
bool DynamixelDriver::readRegister(uint8_t id, const char *item_name, int32_t *data) {
    ...
    if (cti->data_length == BYTE) {
        dxl_comm_result = packetHandler_->read1ByteTxRx(portHandler_, id, cti->address, (uint8_t *) &value_8_bit,
                                                        &error);
    } else if (cti->data_length == WORD) {
        dxl_comm_result = packetHandler_->read2ByteTxRx(portHandler_, id, cti->address, (uint16_t *) &value_16_bit,
                                                        &error);
    } else if (cti->data_length == DWORD) {
        dxl_comm_result = packetHandler_->read4ByteTxRx(portHandler_, id, cti->address, (uint32_t *) &value_32_bit,
                                                        &error);
    }
...
bool DynamixelDriver::lookupLoadedRegisterValue(uint8_t *read_register_array, const char *addr_name, int32_t *value,
                                                uint8_t id) {
    ...
    if (cti->data_length == 1) {
        ...
        *value = DXL_MAKEDWORD(DXL_MAKEWORD(read_register_array[cti->address], read_register_array[cti->address + 1]),
                               DXL_MAKEWORD(read_register_array[cti->address + 2],
                                            read_register_array[cti->address + 3]));
    }
...
bool DynamixelDriver::syncRead(const char *item_name, std::vector<std::vector<int32_t>> &data) {
    int dxl_comm_result = COMM_RX_FAIL;
    bool dxl_addparam_result = false;
...
int32_t DynamixelDriver::convertRadian2Value(uint8_t id, float radian) {
    ...
    if (radian > 0) {
        value = (radian *
                 (tools_[factor].getValueOfMaxRadianPosition() - tools_[factor].getValueOfZeroRadianPosition()) /
                 tools_[factor].getMaxRadian()) + tools_[factor].getValueOfZeroRadianPosition();
    } else if (radian < 0) {
        value = (radian *
                 (tools_[factor].getValueOfMinRadianPosition() - tools_[factor].getValueOfZeroRadianPosition()) /
                 tools_[factor].getMinRadian()) + tools_[factor].getValueOfZeroRadianPosition();
    } else {
...
float DynamixelDriver::convertValue2Radian(uint8_t id, int32_t value) {
    ...
    if (value > tools_[factor].getValueOfZeroRadianPosition()) {
        radian = (float) (value - tools_[factor].getValueOfZeroRadianPosition()) * tools_[factor].getMaxRadian() /
                 (float) (tools_[factor].getValueOfMaxRadianPosition() - tools_[factor].getValueOfZeroRadianPosition());
    } else if (value < tools_[factor].getValueOfZeroRadianPosition()) {
        radian = (float) (value - tools_[factor].getValueOfZeroRadianPosition()) * tools_[factor].getMinRadian() /
                 (float) (tools_[factor].getValueOfMinRadianPosition() - tools_[factor].getValueOfZeroRadianPosition());
    }

    return radian;
}

int32_t DynamixelDriver::convertRadian2Value(float radian, int32_t max_position, int32_t min_position, float max_radian,
                                             float min_radian) {
    int32_t value = 0;
...
float DynamixelDriver::convertValue2Radian(int32_t value, int32_t max_position, int32_t min_position, float max_radian,
                                           float min_radian) {
    float radian = 0.0;
    int32_t zero_position = (max_position + min_position) / 2;
